const result=document.getElementById("resultpara");
...
form.addEventListener("submit",function(e){
    e.preventDefault();
    temp=parseInt(document.getElementById("inputpara").value);
    
    if(isNaN(temp)||temp<=0)
    {
        const n=Math.floor(Math.random()*text.length);
        const t=text[n];
        result.innerHTML=t;
        console.log(text.length);
        alert("Invalid entry!!! Still a paragraph is generated")
    }
    else if(temp>9)
    {
        var t=text.slice(0,9);
        t=t.map(function(item){
            return `${item}`;
        }).join("<br>");
        result.innerHTML=t;
        alert("Max 9 paragraphs are allowed!!!");
    }
    else
    {
        var string='';
